...
namespace LogWhatever.MvcApplication.Controllers.Templates
{
	...
	public class PagesController : BaseController
	{
		...
		private IEnumerable<DashboardMeasurementModel> GetMeasurementValues(IEnumerable<Measurement> measurements, Log log, IEnumerable<MeasurementValue> measurementValues)
		{
			return measurements.Where(x => x.LogId == log.Id).Select(x => new DashboardMeasurementModel { Name = x.Name, Quantity = GetLatestMeasurementValueQuantity(x, measurementValues), Unit = x.Unit });
		}
		...
		public class DashboardLogModel
		{
			...
			public IEnumerable<DashboardMeasurementModel> Measurements { get; set; }
			public IEnumerable<TagEvent> Tags { get; set; } 
			#endregion
		}

		public class DashboardMeasurementModel
		{
			#region Properties
			public string Name { get; set; }
